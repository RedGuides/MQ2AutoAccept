...
#include "ISXEQAutoAccept.h"

// The mandatory pre-setup function.  Our name is "ISXEQAutoAccept", and the class is ISXEQAutoAccept.
...
// our instanced class.
ISXPreSetup("ISXEQAutoAccept",ISXEQAutoAccept);
...
unsigned int ISXEQAutoAcceptXML=0;
...
bool ISXEQAutoAccept::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQAutoAccept Loaded");
return true;
}

// shutdown sequence
void ISXEQAutoAccept::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQAutoAcceptXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQAutoAcceptXML);
...
void ISXEQAutoAccept::ConnectServices()
{
...
void ISXEQAutoAccept::RegisterCommands()
{
    // add any commands
...
void ISXEQAutoAccept::RegisterAliases()
{
    // add any aliases
}

void ISXEQAutoAccept::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQAutoAccept::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQAutoAccept::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQAutoAccept::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQAutoAccept::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQAutoAccept::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQAutoAcceptService)
    //        pISInterface->ShutdownService(this,hISXEQAutoAcceptService);
}

void ISXEQAutoAccept::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQAutoAcceptXML
    ISXEQAutoAcceptXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQAutoAcceptXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQAutoAcceptXML,"General"))
    {
